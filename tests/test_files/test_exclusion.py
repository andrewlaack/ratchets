...
def test_exclusion():
    ...
    for filename in os.listdir(excluded_directory):
        ...
        if not filename in expected_results:
            raise Exception("An additional excluded.txt file was added, but the corresponding expected count was not add to the dict")
    ...
    if count != len(expected_results):
        raise Exception("There is an entry in the expected_results dictionary that does not correspond with a file tested")
