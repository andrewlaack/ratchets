...
def test_files():
    ...
    exceptions1 = run_tests.evaluate_tests(toml_file, False, False, [filtered1_file], True)

    exceptions2 = run_tests.evaluate_tests(toml_file, False, False, [filtered2_file], True)
    ...
    if exception2_sum != 8:
        raise Exception(f"Incorrect number of infractions from {filtered2_file}")
    if exception1_sum != 6:
        raise Exception(f"Incorrect number of infractions from {filtered1_file}")
